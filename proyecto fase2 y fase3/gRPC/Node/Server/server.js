...
var jsonTexto = '{"game_id": "'+ id_juego + '", "players": "'+ call.request.players + '", "game_name" :"'+ game_name + '", "winner" :"'+ ganador + '", "queue" : "Rabbit" }' 
...
function main() {
  ...
  server.bindAsync(process.env.HOST, grpc.ServerCredentials.createInsecure(), () => {
    server.start();
    console.log(`Server esta corriendo en `+process.env.HOST);
  });
}
