...
func imprimir() {
	...
	conn, err := amqp.Dial("amqp://guest:guest@" + getEnvVar("AMQP_HOST") + "/")
	...
	go func() {
		for delivery := range chDelivery {
			fmt.Println("Message Ingresado: " + string(delivery.Body))

			c := KeyBasic{}
			// decodificar el contenido del json sobre la estructura
			err = json.Unmarshal(delivery.Body, &c)
			if err != nil {
				log.Fatal(err)
			}
			...
			buf.ReadFrom(bytes.NewBuffer(delivery.Body))
			str := buf.String()
	...
	//	time.AfterFunc(5*time.Second, imprimir)
}
...
func main() {
	...
	log.Println("Listening at port " + getEnvVar("PRODUCER_PORT"))
	log.Fatal(http.ListenAndServe(":"+getEnvVar("PRODUCER_PORT"), router))
}
